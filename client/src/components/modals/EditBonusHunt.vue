...
const props = defineProps<{ activeHunt: { start: number; name: string } }>();

const name = ref(props.activeHunt.name);
const start = ref(props.activeHunt.start);

const emit = defineEmits<{
  (e: "confirm", name: string, start: number): void;
  (e: "cancel"): void;
}>();
</script>
<template>
  <Modal>
    <h1 class="text-center text-2xl mb-4">Edit bonushunt</h1>
    <!-- <slot /> -->
